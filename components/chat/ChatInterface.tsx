  ...
  const sendMessage = async (e: FormEvent) => {
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          message: message,
          // Pass conversation history so the agent has memory
          history: messages,
        }),
      });
  ...
  return (
    <div className="flex flex-col h-[640px] w-full max-w-[560px] bg-[#0a0a0f] rounded-3xl overflow-hidden shadow-2xl shadow-black/60 border border-white/[0.06]">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 flex flex-col gap-4 [scrollbar-width:thin] [scrollbar-color:rgba(255,255,255,0.08)_transparent]">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 text-white/30 text-sm">
            <span className="text-3xl">🌊</span>
            <p>Ask me about your HBAR balance, recent transactions,</p>
            <p>or say &ldquo;send 1 HBAR to 0.0.12345&rdquo;</p>
          </div>
        )}
        ...
        {messages.map((msg, idx) => (
          ...
          >
            ...
            <div
              className={`px-4 py-3 rounded-2xl max-w-[72%] text-sm leading-relaxed whitespace-pre-wrap ${
                msg.role === "user"
